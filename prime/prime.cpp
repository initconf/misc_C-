...
float prime::_sq_root(int n) //Newton–Raphson method
{
...
bool prime::_is_prime(const int n)
{
...
int* prime::school_method(int n, int* as)
{
    int count = *as ;
    int* a = new int[n] ;
...
//int* prime::prime_of_eratosthenes(int n, int* as)
//{
//  
//    int* tmp = new int[n] ;
//    
...
int* prime::upto_prime(int num, int* num_elements)
{
...
void prime::print(const char* s, int as, int* a)
{
...
bool prime::verify(int as, int* a, int ss, int* b)
{
    if (as != ss )
    {
        return false ;
...
int* prime::_populate_factors(int max, int *num_elements)
{
    
    int* _factors = new int [max] ;
    ...
    for (int i = 2; i <=max ; i++)
    {
      // cout << " I " << i << " count " << count <<  " _factors[" << i << "]" << _factors[i] << endl ;
        if (_factors[i] == 0 )
        {
            count ++ ;
          //  cout << " I " << i << " count " << count <<  " _factors[" << i << "]" << _factors[i] << endl ;
